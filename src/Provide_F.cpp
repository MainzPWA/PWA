...
Double_t GetChiSq_F()
{
  ...
  for(Int_t n=0; n<nF; n++) //Process all found bins
  {
    ...
    {
      Theta = F_th[eF[n]][th];
      Meas  = F_sc[eF[n]]*F_val[eF[n]][th]*f_obs[ASY_F];
      Error = F_sc[eF[n]]*F_err[eF[n]][th]*f_obs[ASY_F];
      Theo  = F(Theta, Omega);
...
Double_t GetScale_F()
{
  ...
  for(Int_t n=0; n<nF; n++) //Process all found bins
    Scale_F+=(1.0*F_pts[eF[n]])*(f_obs[ASY_F]-1.0)*(f_obs[ASY_F]-1.0)/(F_sy[eF[n]]*F_sy[eF[n]]);
