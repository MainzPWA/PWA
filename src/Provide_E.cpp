...
Double_t GetChiSq_E()
{
  ...
  for(Int_t n=0; n<nE; n++) //Process all found bins
  {
    ...
    {
      Theta = E_th[eE[n]][th];
      Meas  = E_sc[eE[n]]*E_val[eE[n]][th]*f_obs[SIG_0];
      Error = E_sc[eE[n]]*E_err[eE[n]][th]*f_obs[SIG_0];
      Theo  = E(Theta, Omega);
...
Double_t GetScale_E()
{
  ...
  for(Int_t n=0; n<nE; n++) //Process all found bins
    Scale_E+=(1.0*E_pts[eE[n]])*(f_obs[SIG_0]-1.0)*(f_obs[SIG_0]-1.0)/(E_sy[eE[n]]*E_sy[eE[n]]);
