...
void Parse_Cx()
{
  ...
  while(!feof(File_Cx))
  {
    ...
    while(ReadLine_Cx(File_Cx, &Theta, &Cx, &DCx)==3)
    {
...
Int_t ReadLine_Cx(FILE* File_Cx, Double_t* Theta, Double_t* Cx, Double_t* DCx)
{
  Char_t Buffer[1024];

  fgets(Buffer, sizeof(Buffer), File_Cx);
  return sscanf(Buffer, "%lf %lf %lf", Theta, Cx, DCx);
}
