...
void Parse_Cz()
{
  ...
  while(!feof(File_Cz))
  {
    ...
    while(ReadLine_Cz(File_Cz, &Theta, &Cz, &DCz)==3)
    {
...
Int_t ReadLine_Cz(FILE* File_Cz, Double_t* Theta, Double_t* Cz, Double_t* DCz)
{
  Char_t Buffer[1024];

  fgets(Buffer, sizeof(Buffer), File_Cz);
  return sscanf(Buffer, "%lf %lf %lf", Theta, Cz, DCz);
}
