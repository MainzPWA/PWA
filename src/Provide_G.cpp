...
Double_t GetChiSq_G()
{
  ...
  for(Int_t n=0; n<nG; n++) //Process all found bins
  {
    ...
    {
      Theta = G_th[eG[n]][th];
      Meas  = G_sc[eG[n]]*G_val[eG[n]][th]*f_obs[SIG_0];
      Error = G_sc[eG[n]]*G_err[eG[n]][th]*f_obs[SIG_0];
      Theo  = G(Theta, Omega);
...
Double_t GetScale_G()
{
  ...
  for(Int_t n=0; n<nG; n++) //Process all found bins
    Scale_G+=(1.0*G_pts[eG[n]])*(f_obs[SIG_0]-1.0)*(f_obs[SIG_0]-1.0)/(G_sy[eG[n]]*G_sy[eG[n]]);
